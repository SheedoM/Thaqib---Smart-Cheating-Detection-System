...
def collect_samples():
    """
    Collect audio samples and label them manually (2 classes only).
    Classes:
        - cheating
        - non_cheating
    """
    ...
    print("\n" + "=" * 60)
    print("  Data Collection - 2 Classes System")
    print("=" * 60)

    print("Labels:")
    print("  [c] Cheating 🚨   (whispering / talking)")
    print("  [n] Non-Cheating ✅ (noise / paper / cough)")
    print("\nPress Ctrl+C to stop.\n")

    # Noise calibration
    print("🎤 Calibrating noise... stay silent for 3 seconds")
    noise_thresh = _calibrate_noise(vad)
    print(f"✅ Threshold set: {noise_thresh:.6f}\n")

    sample_count = _count_existing_samples()
    print(f"📁 Existing samples: {sample_count}")
    print("🎙 Start speaking / making sounds...\n")
...
def _print_sample_stats():
    ...
    for label, icon in (("cheating", "🚨"), ("non_cheating", "✅")):
        ...
        bar = "█" * n
        print(f"  {icon} {label:20s} : {n:3d}  {bar}")

def _get_label_input(audio_data):
    COLLECT_CLASSES = {"c": "cheating", "n": "non_cheating"}
    while True:
        ...
        if choice == "s":
            ...
            sd.wait()
        elif choice in COLLECT_CLASSES:
            return COLLECT_CLASSES[choice]
        else:
            print("اختيار غير صحيح، حاول مجدداً.")

def _save_sample(audio_data, label):
    # label هنا إما "cheating" أو "non_cheating"
...
def train_model():
    ...
    for label_name, label_value in label_map.items():
        ...
        print(f"Loading {label_name}: {len(files)} samples")
    ...
    label_names = ["Non-Cheating", "Cheating"]
