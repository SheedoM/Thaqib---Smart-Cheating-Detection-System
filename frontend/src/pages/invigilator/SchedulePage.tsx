...
export default function SchedulePage() {
  ...
  const todaysAssignments = assignments.filter((assignment) => {
    ...
    return start.toLocaleDateString('en-CA') === todayKey;
  });
    ...
    <div className="px-6 py-8">
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            {todaysAssignments.length > 0 ? (
              todaysAssignments.map((assignment) => (
                <AssignmentCard key={assignment.id} assignment={assignment} onOpen={() => navigate(`/invigilator/session/${assignment.exam_session_id}/${assignment.hall_id}`)} />
              ))
            ) : (
              <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200">
                <p className="text-gray-400">لا توجد جلسات مجدولة اليوم.</p>
              </div>
            )}
          </div>
        </section>
        {otherAssignments.length > 0 && (
          <section>
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 px-1">مواعيد أخرى</h3>
            <div className="space-y-4">
