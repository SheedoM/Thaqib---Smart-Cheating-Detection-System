...
class AVAlertComposer:
    ...
    def compose_video_alert(
        ...
        if not video_archive or not os.path.exists(video_archive):
            logger.warning(f"Video archive not found for camera {camera_id}. Cannot compose video alert.")
            return
        ...
        if mic_id:
            ...
            if audio_archive and os.path.exists(audio_archive):
                ...
                logger.warning(f"Audio archive not found for mic {mic_id}. Generating video-only alert.")
        else:
        ...
        if has_audio:
            ...
            if not has_video:
                logger.warning(f"No frames extracted from video archive for {camera_id} at {start_sec}s. Audio alert after video EOF?")
            ...
            video_only_path = os.path.join(self.output_dir, f"{alert_type}_{camera_id}_{timestamp:.1f}.mp4")
    ...
    def compose_audio_alert(self, alert_wav_path: str, mic_id: str, camera_ids: list[str], start_sec: float, end_sec: float):
        ...
        for camera_id in camera_ids:
            ...
            annotated_video_path = os.path.join(self.output_dir, f"temp_video_{camera_id}_{filename}.mp4")
            has_video = self._extract_and_annotate_video(video_archive, annotated_video_path, camera_id, mic_id, start_sec, end_sec)
            
            if not has_video:
                logger.warning(f"No frames extracted from video archive for {camera_id} at {start_sec}s. Audio alert happened after video ended?")
                if os.path.exists(annotated_video_path):
                    os.remove(annotated_video_path)
                continue
